...
let db = { ...EMPTY_DB };
try {
  db = { ...EMPTY_DB, ...JSON.parse(storage?.getItem(KEY) || storage?.getItem("nutrim-vet-v26") || storage?.getItem("nutrim-vet-v25") || storage?.getItem("nutrim-vet-v24") || storage?.getItem("nutrim-vet-v23") || storage?.getItem("nutrim-vet-v22") || storage?.getItem("nutrim-vet-v21") || "{}") };
} catch {
  db = { ...EMPTY_DB };
}
...
function reminderForm(pl) {
  ...
  return `<form class="sheet-form app-form-card" data-save-reminder><label>Titulo<input name="title" required /></label><label>Tipo<select name="type"><option>Enviar fotos</option><option>Control de peso</option><option>Revisar tolerancia digestiva</option><option>Ajustar gramos</option><option>Proximo turno nutricional</option><option>Otro</option></select></label><label>Fecha programada<input name="date" type="date" required /></label><label>Descripcion<textarea name="description" rows="4"></textarea></label><label class="check-line"><input name="visible" type="checkbox" /> Visible para tutor</label><div class="form-actions"><button class="primary-button">Guardar recordatorio</button></div></form><section class="embedded-list"><span class="eyebrow">Recordatorios del plan</span>${list.length ? list.map((r) => `<article class="sheet-list-item"><span><strong>${esc(r.title)}</strong><small>${formatDate(r.scheduledDate)} · ${esc(r.status)}</small>${r.description ? `<p>${esc(r.description)}</p>` : ""}</span><span class="inline-actions"><button class="ghost-button compact" data-reminder-status="${r.id}" data-status="cumplido">Cumplido</button><button class="ghost-button compact" data-reminder-status="${r.id}" data-status="cancelado">Cancelar</button></span></article>`).join("") : "<p class='muted'>Todavia no hay recordatorios reales para este plan.</p>"}</section>`;
}
