...
let db = { ...EMPTY_DB };
try {
  db = { ...EMPTY_DB, ...JSON.parse(storage?.getItem(KEY) || storage?.getItem("nutrim-vet-v23") || storage?.getItem("nutrim-vet-v22") || storage?.getItem("nutrim-vet-v21") || "{}") };
} catch {
  db = { ...EMPTY_DB };
}
...
function mealOptions(selected = []) {
  const options = ["No se indica", ...db.foods.map((x) => `Alimento: ${x}`), ...db.supplements.map((x) => `Suplemento: ${x}`), "Otro alimento", "Otro suplemento"];
  return options.map((x) => `<option value="${esc(x)}" ${selected.includes(x) ? "selected" : ""}>${esc(x)}</option>`).join("");
}
function mealEditor(key, label, stage = {}) {
  const meal = mealFor(stage, key);
  return `<fieldset class="meal-editor"><legend>${label}</legend><select name="${key}Items" multiple size="6" data-other-toggle="${key}Other">${mealOptions(meal.selected)}</select><small class="field-hint">Selecciona alimentos, suplementos o "No se indica".</small><label class="${meal.showOtherFood ? "" : "is-hidden"}" data-other-field="${key}Other">Otro alimento<input name="${key}OtherFood" value="${esc(meal.otherFood)}" /></label><label class="${meal.showOtherSupplement ? "" : "is-hidden"}" data-other-field="${key}Other">Otro suplemento<input name="${key}OtherSupplement" value="${esc(meal.otherSupplement)}" /></label><label>Detalle / cantidad<textarea name="${key}Notes" rows="3">${esc(meal.notes)}</textarea></label></fieldset>`;
}
function mealFor(stage, key) {
  const meal = stage.meals?.[key] || {};
  const legacy = stage[key] && !stage.meals ? stage[key] : "";
  const selected = [...(meal.noIndicated ? ["No se indica"] : []), ...(meal.foods || []).map((x) => `Alimento: ${x}`), ...(meal.supplements || []).map((x) => `Suplemento: ${x}`)];
...
function mealFromForm(d, key) {
  ...
  return {
    noIndicated: values.includes("No se indica"),
    foods: unique([...values.filter((x) => x.startsWith("Alimento: ")).map((x) => x.replace("Alimento: ", "")), otherFood]),
    supplements: unique([...values.filter((x) => x.startsWith("Suplemento: ")).map((x) => x.replace("Suplemento: ", "")), otherSupplement]),
