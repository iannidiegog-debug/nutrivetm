...
let db = { ...EMPTY_DB };
try {
  db = { ...EMPTY_DB, ...JSON.parse(storage?.getItem(KEY) || storage?.getItem("nutrim-vet-v25") || storage?.getItem("nutrim-vet-v24") || storage?.getItem("nutrim-vet-v23") || storage?.getItem("nutrim-vet-v22") || storage?.getItem("nutrim-vet-v21") || "{}") };
} catch {
  db = { ...EMPTY_DB };
}
...
db.foods = unique(db.foods || []);
db.supplements = unique(db.supplements || []);
...
function sheet(pl) {
  const name = { newPlan: "Crear plan", general: "Datos generales", stages: "Etapas", newStage: "Nueva etapa", editStage: "Editar etapa", ingredients: "Biblioteca alimentos", supplements: "Biblioteca suplementos", reminderForm: "Recordatorio", message: "Mensaje tutor", preview: "Vista previa tutor" }[state.sheet];
...
function bind() {
  ...
  $("[data-export-excel]")?.addEventListener("click", exportPlanExcel);
...
function saveReminder(e) { e.preventDefault(); const d = new FormData(e.currentTarget); const item = createReminder(patient().id, text(d, "type"), text(d, "title"), text(d, "description"), text(d, "date"), d.get("visible") === "on", "manual", plan()?.id); addHistory(patient().id, "reminder", item.id, "Recordatorio creado", item.title, "profesional", item.visibleForTutor); state.sheet = ""; save(); render(); }
...
function historyTimeline(patientId) {
  ...
  return `<section class="history-block"><div class="section-heading compact-heading"><div><span class="eyebrow">Historia nutricional</span><h3>Linea de tiempo</h3></div></div>${items.length ? `<div class="timeline">${items.map((h) => `<article><span>${formatDateTime(h.date)}</span><strong>${esc(h.title)}</strong><p>${esc(h.description || "")}</p></article>`).join("")}</div>` : empty("Este paciente todavia no tiene historia nutricional.", "Los eventos apareceran cuando se carguen datos reales.")}</section>`;
}
...
function tutorReportView() {
  ...
  return `<div class="content-grid"><section class="panel wide"><div class="section-heading"><div><span class="eyebrow">${esc(p.name)}</span><h2>Reporte del dia</h2></div></div><form class="sheet-form" data-tutor-report><label>Fecha del reporte<input name="date" type="date" value="${todayIso()}" /></label><div class="mini-grid">${selectField("ate", "Comio bien?", ["Si", "No", "Parcial"])}${selectField("ration", "Termino la racion indicada?", ["Si", "No", "Parcial"])}${selectField("rejected", "Rechazo el alimento?", ["No", "Si"])}${selectField("vomit", "Tuvo vomitos?", ["No", "Si"])}${selectField("diarrhea", "Tuvo diarrea?", ["No", "Si"])}${selectField("stool", "Materia fecal", ["Normal", "Blanda", "Diarrea", "Constipacion", "Con sangre", "No observada"])}${selectField("energy", "Estado general", ["Normal", "Activo", "Decaido"])}</div><label>Observaciones del tutor<textarea name="observations" rows="4"></textarea></label><label>Fotos del seguimiento<input name="photos" type="file" multiple accept="image/*,.pdf,.doc,.docx" /></label><div class="form-actions"><button class="ghost-button" type="button" data-home>Cancelar</button><button class="primary-button">Guardar reporte</button></div></form></section><section class="panel"><span class="eyebrow">Reportes anteriores</span>${reports.length ? reports.map((r) => `<article class="mini-record"><strong>${formatDate(r.date)}</strong><p>${esc(r.ate)} · ${esc(r.stool)} · ${esc(r.energy)}</p></article>`).join("") : empty("Todavia no hay reportes del tutor.", "Cuando guardes un reporte aparecera aca.")}</section></div>`;
}
...
function reminderForm(pl) {
  return `<form class="sheet-form" data-save-reminder><label>Titulo<input name="title" required /></label><label>Tipo<select name="type"><option>Enviar fotos</option><option>Control de peso</option><option>Revisar tolerancia digestiva</option><option>Ajustar gramos</option><option>Proximo turno nutricional</option><option>Otro</option></select></label><label>Fecha programada<input name="date" type="date" required /></label><label>Descripcion<textarea name="description" rows="4"></textarea></label><label class="check-line"><input name="visible" type="checkbox" /> Visible para tutor</label><div class="form-actions"><button class="primary-button">Guardar recordatorio</button></div></form>`;
}
