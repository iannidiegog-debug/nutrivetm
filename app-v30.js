...
let db = { ...EMPTY_DB };
try {
  db = { ...EMPTY_DB, ...JSON.parse(storage?.getItem(KEY) || storage?.getItem("nutrim-vet-v29") || storage?.getItem("nutrim-vet-v28") || storage?.getItem("nutrim-vet-v27") || storage?.getItem("nutrim-vet-v26") || storage?.getItem("nutrim-vet-v25") || storage?.getItem("nutrim-vet-v24") || storage?.getItem("nutrim-vet-v23") || storage?.getItem("nutrim-vet-v22") || storage?.getItem("nutrim-vet-v21") || "{}") };
} catch {
  db = { ...EMPTY_DB };
}
...
function patientForm() {
  return `<form class="panel wide patient-form" data-patient-form><div class="section-heading"><div><span class="eyebrow">Alta de paciente</span><h2>Nuevo paciente veterinario</h2></div><button class="ghost-button" type="button" data-cancel>Volver</button></div><div class="form-grid"><section class="form-section"><h3>Paciente</h3><label>Nombre<input name="name" required /></label><label>Especie<input name="species" required placeholder="Ej: canino, felino, ave, reptil, equino..." /></label><label>Raza o tipo<input name="breed" /></label><div class="mini-grid"><label>Edad<input name="age" /></label><label>Sexo<select name="sex"><option value="">Seleccionar</option><option>Macho</option><option>Hembra</option><option>No aplica</option></select></label></div><label>Estado reproductivo<select name="neuteredStatus"><option value="">Seleccionar</option><option>Castrado</option><option>No castrado</option><option>No aplica</option></select></label><div class="mini-grid"><label>Peso actual<input name="weight" type="number" step="0.1" /></label><label>Condicion corporal<input name="bodyCondition" /></label></div><label>Alimentacion actual<textarea name="currentFood" rows="3"></textarea></label><label>Observaciones clinicas visibles<textarea name="visibleClinicalNotes" rows="3"></textarea></label><label>Observaciones internas<textarea name="internalClinicalNotes" rows="3"></textarea></label></section><section class="form-section"><h3>Tutor</h3><label>Nombre y apellido<input name="tutor" required /></label><label>DNI<input name="dni" /></label><label>Email<input name="email" type="email" /></label><label><input name="tutorAccess" type="checkbox" /> Habilitar acceso del tutor</label></section></div><div class="form-actions"><button class="ghost-button" type="button" data-cancel>Cancelar</button><button class="primary-button">Guardar paciente</button></div></form>`;
}
