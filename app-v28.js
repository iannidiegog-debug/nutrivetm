...
const KEY = "nutrim-vet-v28";
const EMPTY_DB = { patients: [], plans: [], stages: [], docs: [], alerts: [], foods: [], supplements: [], tutorReports: [], history: [], reminders: [] };
const storage = (() => {
  try {
...
let db = { ...EMPTY_DB };
try {
  db = { ...EMPTY_DB, ...JSON.parse(storage?.getItem(KEY) || storage?.getItem("nutrim-vet-v27") || storage?.getItem("nutrim-vet-v26") || storage?.getItem("nutrim-vet-v25") || storage?.getItem("nutrim-vet-v24") || storage?.getItem("nutrim-vet-v23") || storage?.getItem("nutrim-vet-v22") || storage?.getItem("nutrim-vet-v21") || "{}") };
} catch {
  db = { ...EMPTY_DB };
}
...
const defaultFoods = [
  "Alimento balanceado seco", "Alimento balanceado urinario", "Alimento balanceado para obesidad", "Alimento balanceado diabetes", "Alimento balanceado cachorros", "Alimento balanceado adultos", "Alimento balanceado geriatricos",
  "Pollo", "Pechuga de pollo", "Muslo de pollo", "Pavo", "Carne vacuna magra", "Nalga", "Bola de lomo", "Lomo", "Roast beef magro", "Cerdo magro", "Pescado blanco", "Merluza", "Lenguado", "Atun al natural", "Salmon", "Trucha", "Sardina", "Huevo entero", "Clara de huevo",
  "Pollo crudo", "Pavo crudo", "Carne vacuna cruda", "Cordero crudo", "Cerdo apto para BARF", "Pescado crudo", "Huesos carnosos de pollo", "Carcasa de pollo", "Alas de pollo", "Costillas carnosas", "Corazon bovino", "Corazon de pollo", "Mollejas", "Mondongo verde", "Traquea bovina",
  "Higado vacuno", "Higado de pollo", "Rinon", "Bazo", "Pulmon", "Sesos", "Pancreas", "Corazon",
  "Zanahoria", "Zapallo", "Calabaza", "Batata", "Boniato", "Zucchini", "Zapallito", "Pepino", "Brocoli", "Coliflor", "Repollo", "Espinaca", "Acelga", "Rucula", "Lechuga", "Chauchas", "Arvejas", "Remolacha", "Apio",
  "Manzana", "Pera", "Banana", "Frutilla", "Arandanos", "Mora", "Sandia", "Melon", "Mango", "Papaya", "Durazno", "Kiwi", "Coco",
  "Arroz blanco", "Arroz integral", "Avena", "Quinoa", "Mijo", "Cebada", "Fideos", "Papa", "Yuca", "Polenta",
  "Aceite de pescado", "Aceite de salmon", "Aceite de krill", "Aceite de sardina", "Aceite de oliva", "Aceite de coco", "Grasa vacuna", "Manteca clarificada (ghee)",
  "Yogur natural", "Yogur griego", "Kefir", "Queso cottage", "Ricota magra",
  "Semillas de chia", "Semillas de lino", "Hemp (canamo)", "Alga kelp", "Alga spirulina", "Arandanos deshidratados", "Coco rallado"
];
const defaultSupplements = [
  "Omega 3 EPA/DHA", "Omega 6", "Omega 9", "Aceite de pescado", "Aceite de salmon", "Calcio", "Carbonato de calcio", "Citrato de calcio", "Calcio de cascara de huevo", "Fosforo", "Magnesio", "Zinc", "Hierro", "Selenio", "Cobre", "Manganeso", "Yodo",
  "Glutamina", "Taurina", "Carnitina", "Lisina", "Arginina", "Metionina", "BCAA", "Colageno hidrolizado",
  "Glucosamina", "Condroitina", "MSM", "Colageno tipo II", "Acido hialuronico", "Mejillon de labios verdes",
  "Probioticos", "Prebioticos", "Inulina", "Enterococcus faecium", "Saccharomyces boulardii", "Enzimas digestivas", "Fibra soluble", "Psyllium",
  "Multivitaminico", "Vitamina A", "Vitamina D", "Vitamina E", "Vitamina K", "Complejo B", "Vitamina B12", "Acido folico", "Vitamina C",
  "Calostro bovino", "Beta glucanos", "Hongos medicinales", "Equinacea", "Astragalo",
  "Curcuma", "Jengibre", "Espirulina", "Clorela", "Aloe vera", "Levadura de cerveza", "Polen de abeja", "Propoleo", "Harina de coco"
];
...
function planMenu(pl) {
  ...
  const items = [["general", "Datos generales", pl.objective], ["stages", "Etapas", currentStage ? `Activa: ${currentStage.name}` : stages().length ? `${stages().length} etapa(s)` : ""], ["ingredients", "Biblioteca alimentos", stageIngredients.join(", ")], ["supplements", "Biblioteca suplementos", stageSupplements.join(", ")], ["reminderForm", "Recordatorios", reminderStatus], ["message", "Mensaje tutor", pl.message], ["preview", "Vista previa tutor", currentStage?.name]];
  return `<div class="plan-console-header"><div><span class="badge">${pl.status}</span><h3>${pl.title}</h3><p>${pl.type || "Tipo pendiente"} · ${pl.duration || "Duracion pendiente"}</p>${currentStage ? `<small>Etapa actual: ${esc(currentStage.name)}</small>` : ""}</div><div class="plan-header-actions"><button class="primary-button compact" data-export-pdf>Exportar PDF</button><button class="ghost-button compact" data-export-excel>Exportar Excel</button><button class="ghost-button compact danger-text" data-delete-plan>Eliminar plan</button></div></div><div class="plan-section-menu">${items.map(([k, t, v]) => `<button class="plan-section-button" data-section="${k}"><span><strong>${t}</strong><small>${v || "Pendiente"}</small></span><em class="${v ? "ready" : ""}">${v ? "Cargado" : "Pendiente"}</em></button>`).join("")}</div>`;
}

function sheet(pl) {
  const name = { newPlan: "Crear plan", general: "Datos generales", stages: "Etapas", newStage: "Nueva etapa", editStage: "Editar etapa", ingredients: "Biblioteca alimentos", supplements: "Biblioteca suplementos", reminderForm: "Crear recordatorio", reminderList: "Recordatorios del plan", message: "Mensaje tutor", preview: "Vista previa tutor" }[state.sheet];
...
function savePatient(e) { e.preventDefault(); const d = new FormData(e.currentTarget); if (!text(d, "name") || !text(d, "species") || !text(d, "tutor") || (!text(d, "dni") && !text(d, "email"))) return alert("Completa animal, especie, tutor y DNI o email."); const p = { id: id(), tutorId: text(d, "dni") || text(d, "email"), name: text(d, "name"), species: text(d, "species"), breed: text(d, "breed"), weight: text(d, "weight"), targetWeight: text(d, "targetWeight"), allergies: text(d, "allergies"), medication: text(d, "medication"), tutor: text(d, "tutor"), dni: text(d, "dni"), email: text(d, "email"), tutorAccess: d.get("tutorAccess") === "on", status: "Pendiente plan" }; db.patients.unshift(p); state.patientId = p.id; state.sheet = ""; addHistory(p.id, "patient", p.id, "Paciente creado", `Se creo el legajo de ${p.name}.`, "profesional", false); save(); render(); }
function savePlan(e) { e.preventDefault(); const d = new FormData(e.currentTarget); const type = text(d, "type"); const item = { id: id(), patientId: patient().id, tutorId: patient().tutorId, title: text(d, "title"), type, typeOther: type === "Otro" ? text(d, "typeOther") : "", objective: text(d, "objective"), startDate: text(d, "startDate"), duration: text(d, "duration"), status: text(d, "status"), nextControl: text(d, "nextControl"), ingredients: [], supplements: [], supplementSchedule: {}, published: false }; db.plans.unshift(item); addHistory(patient().id, "nutrition_plan", item.id, "Plan alimentario creado", item.title, "profesional", false); if (item.nextControl) createReminder(patient().id, "control", "Proximo control nutricional", item.objective || item.title, item.nextControl, true, "plan", item.id); state.sheet = ""; save(); render(); }
function saveSection(e) {
  e.preventDefault();
  const d = new FormData(e.currentTarget), p = plan();
  if (state.sheet === "general") { const type = text(d, "type"); Object.assign(p, { title: text(d, "title"), type, typeOther: type === "Otro" ? text(d, "typeOther") : "", objective: text(d, "objective"), startDate: text(d, "startDate"), duration: text(d, "duration"), status: text(d, "status"), nextControl: text(d, "nextControl") }); addHistory(p.patientId, "nutrition_plan", p.id, "Plan alimentario modificado", p.title, "profesional", false); if (p.nextControl) createReminder(p.patientId, "control", "Proximo control nutricional", p.objective || p.title, p.nextControl, true, "plan", p.id); }
...
function deletePatient() {
  ...
  const planIds = new Set(db.plans.filter((pl) => patientIds.has(pl.patientId)).map((pl) => pl.id));
  db.patients = db.patients.filter((item) => !patientIds.has(item.id));
  db.plans = db.plans.filter((item) => !patientIds.has(item.patientId));
...
function deletePlan() {
  ...
  db.plans = db.plans.filter((item) => item.id !== pl.id);
...
function publishPlan() {
  ...
  pl.published = true;
  p.tutorAccess = true;
  addHistory(p.id, "nutrition_plan", pl.id, "Plan alimentario publicado al tutor", pl.title, "profesional", true);
  save();
...
function activePlan(patientId = state.patientId) { return db.plans.find((p) => p.patientId === patientId && p.status === "activo") || db.plans.find((p) => p.patientId === patientId) || null; }
...
function tutorHomeView() {
  const p = tutorPatient(), pl = activePlan(p.id), stage = activeStage(pl);
...
function tutorFullPlanView() {
  const p = tutorPatient(), pl = activePlan(p.id);
